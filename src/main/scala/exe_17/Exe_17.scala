...
object Exe_17 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def parse_input(data: Vector[String]): Set[(Int, Int, Int)] = {
      (data.zipWithIndex flatMap { y => y._1.zipWithIndex filter { x => x._1 == '#' } map { x => (x._2, y._2, 0) } }).toSet
    }

    def get_surrounding(cube: (Int, Int, Int)): List[(Int, Int, Int)] = {
      val (x,y,z) = cube
      (for {
        a <- -1 to 1
        b <- -1 to 1
        c <- -1 to 1
        if (a,b,c) != (0,0,0)
      } yield (a+x, b+y, c+z)).toList
    }

    def deactivate(implicit active_cubes: Set[(Int, Int, Int)]): Set[(Int, Int, Int)] = {

      def deactivate_cube(cube: (Int, Int, Int)): Boolean = {
    ...
    def activate(implicit active_cubes: Set[(Int, Int, Int)]): Set[(Int, Int, Int)] = {
    ...
    def cycle(implicit active_cubes: Set[(Int, Int, Int)]): Set[(Int, Int, Int)] = {
      deactivate union activate
    }

    def run(n_cyc: Int, active_cubes: Set[(Int, Int, Int)]): Int = {
